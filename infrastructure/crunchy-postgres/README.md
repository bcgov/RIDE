...
- Your folder structure is `C:\Data\RIDE` and you have the latest version of repo copied to that location (You can use a different location if you like)

1. Login to `oc` 
1. Confirm you are in the correct namespace and cluster by entering `oc project`
1. Run `cd C:\Data\RIDE\`
1. Confirm the values file has all values set as you like. Then fill in any of these values below as required:
1.        helm install ENV-ride-db \
...
1. On the Standby Cluster, run `oc edit secret ENV-ride-db-crunchy-monitoring` on the Standby Cluster (or just go there via the UI)
