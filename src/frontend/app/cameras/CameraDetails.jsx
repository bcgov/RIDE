...
export default function CameraDetails({ onBack }) {
  ...
  // View selected before arriving at the details page.
  // Can come from:
  //   /cameras/123?view=456
  // or router state:
  //   navigate('/cameras/123', { state: { selectedViewId: 456 } })
  const initialViewId =
  ...
  //
  // This is intentionally separate from the camera data because
  // clicking a thumbnail should only change the displayed image,
  // not modify the camera itself.
  ...
  const handleSave = async () => {
    try {
      const payload = {
        ...
        business_area_id: basicsData.businessArea
          ? Number(basicsData.businessArea)
          : null,

        region_id: basicsData.region
          ? Number(basicsData.region)
          : null,

        road_id: basicsData.road
          ? Number(basicsData.road)
          : null,

        camera_type_id: setupData.cameraType
          ? Number(setupData.cameraType)
          : null,

        camera_make_id: setupData.cameraMake
          ? Number(setupData.cameraMake)
          : null,

        connection_type_id: setupData.connectionType
          ? Number(setupData.connectionType)
          : null,

        connection_protocol_id: setupData.connectionProtocol
          ? Number(setupData.connectionProtocol)
          : null,

        communication_type_id: setupData.communicationType
          ? Number(setupData.communicationType)
          : null,

        power_source_id: setupData.powerSource
          ? Number(setupData.powerSource)
          : null,

        communication_device_id: setupData.communicationDevice
          ? Number(setupData.communicationDevice)
          : null,

        antenna_id: setupData.antenna
          ? Number(setupData.antenna)
          : null,

        service_provider_id: setupData.serviceProvider
          ? Number(setupData.serviceProvider)
          : null,
        ...
        road_maintenance_contractor_id:
          basicsData.roadMaintenanceContractor
            ? Number(basicsData.roadMaintenanceContractor)
            : null,

        electrical_contractor_id:
          basicsData.electricalContractor
            ? Number(basicsData.electricalContractor)
            : null,

        locations_geo_latitude:
          basicsData.latitude
            ? Number(basicsData.latitude)
            : null,

        locations_geo_longitude:
          basicsData.longitude
            ? Number(basicsData.longitude)
            : null,

        locations_elevation:
          basicsData.elevation
            ? Number(basicsData.elevation)
            : null,
        ...
        views: viewsData.map((v, index) => ({
          id:
            v.id &&
            !isNaN(Number(v.id)) &&
            !String(v.id).startsWith('new-')
              ? Number(v.id)
              : undefined,

          orientation: v.orientation
            ? v.orientation.toUpperCase()
            : '',
  ...
  const handleViewSelect = (viewId) => {
    ...
    // Keep the selected view in the URL.
    //
    // This means refreshing:
    // /cameras/123?view=456
    //
    // will keep view 456 selected.
    setSearchParams(
      (prev) => {
  ...
  const [notesData, setNotesData] = useState([
    {
      ...
      content:
        'Camera modem replaced on Jan 21, 2026',
    },
    {
      id: '2',
  ...
  const [historyData] = useState([
    {
      id: '1',
      timestamp:
        'Thurs, May 22, 2026 10:00 am PST',
      user: 'David Jupp',
      sections: [
        {
          category: 'Visibility',
      ...
      sections: [
        {
          ...
          actions: [
            ...
            {
              type: 'add',
              text: 'Added modem serial number',
            },
            {
              type: 'remove',
              text: 'Removed antennae',
            },
          ],
        },
        {
          ...
          actions: [
            {
              type: 'add',
              text: 'Added view ID for north',
            },
            {
              type: 'add',
              text: 'Added path for north',
            },
          ],
        },
      ],
