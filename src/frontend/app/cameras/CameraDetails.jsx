...
export default function CameraDetails({ onBack }) {
  ...
  const previousViewTimestamps = useRef({}); // { [viewId]: { last_update_attempt, last_update_modified } }
  ...
  const handleToggleView = (view) => {
    if (view.is_on) {
      // Turning off — collect the disable reason first
      setDisableModalTarget(view);
      return;
    }

    // Turning on — no form needed, toggle immediately
    performToggleView(view, true, () => {
      setToast({ message: 'View enabled', variant: 'success' });
    });
  };

  const performToggleView = async (view, newIsOn, onSuccess, extraFields = {}) => {
    // Optimistic update — camera.views is what the cards actually render from
    setCamera((prev) => ({
      ...prev,
      views: (prev.views || []).map((v) =>
        v.id === view.id ? { ...v, is_on: newIsOn, ...extraFields } : v
      ),
    }));
  ...
  const handleConfirmDisable = async ({ reason, shortDescription, longDescription }) => {
    const view = disableModalTarget;

    await performToggleView(
      view,
      false,
      () => {
        setDisableModalTarget(null);
        setToast({ message: 'View disabled', variant: 'success' });
      },
      {
  ...
  const createLogEntry = async (cameraId, viewId, message, isError) => {
    try {
      await fetch(`/api/cameras/${cameraId}/logs/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-CSRFToken': getCookie('csrftoken'),
  ...
  const buildViewsPayload = (views) => {
    return views.map((view, index) => ({
      ...
      display_order: index,
      last_update_attempt: view.last_update_attempt,
      last_update_modified: view.last_update_modified,
    }));
  };
  ...
  const loadDriveBCWebcam = async (id) => {
    const response = await fetch(`/drivebc-api/api/webcams/${id}/`);
    if (!response.ok) {
      throw new Error(`Failed to load DriveBC webcam ${id}: ${response.status}`);
    }
  ...
  const loadCamera = async () => {
      try {
        ...
        // For each view that references a DriveBC webcam, fetch that webcam's
        // live status and merge in last_update_modified / last_update_attempt.
        const mergedViews = await Promise.all(
          (data.views || []).map(async (view) => {
            ...
            try {
              const webcam = await loadDriveBCWebcam(view.drivebc_webcam_id);
              const prev = previousViewTimestamps.current[view.id];

              const attemptChanged = prev && prev.last_update_attempt !== webcam.last_update_attempt;
              const modifiedChanged = prev && prev.last_update_modified !== webcam.last_update_modified;

              if (!isFirstLoad.current && attemptChanged) {
                const label = orientationLabel(view.orientation);
                if (modifiedChanged) {
                  createLogEntry(id, view.id, `Image received - ${label}`, false);
                } else {
                  createLogEntry(id, view.id, `No image received - ${label}`, true);
                }
              }

              previousViewTimestamps.current[view.id] = {
                last_update_attempt: webcam.last_update_attempt,
                last_update_modified: webcam.last_update_modified,
              };
              ...
              console.error(`Failed to load DriveBC webcam ${view.drivebc_webcam_id}:`, err);
              return view;
            }
          })
        );
  ...
  const renderViewCard = (view, { expanded = false } = {}) => {
    ...
    const formattedTime = view.last_update_modified
      ? new Date(view.last_update_modified).toLocaleTimeString('en-US', {
          hour: 'numeric',
          minute: '2-digit',
          timeZoneName: 'short',
        })
  ...
  const viewsList = [...(camera?.views || [])]
    .filter((view) => !!(view.image_url || view.drivebc_webcam_id))
    .sort((a, b) => (a.display_order ?? 0) - (b.display_order ?? 0));
