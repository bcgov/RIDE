...
import { faCheckCircle, faClock, faVideoSlash } from '@fortawesome/pro-regular-svg-icons';
  ...
  return (
    <div className="camera-card">
      ...
      >
        ...
        {/* Stale/Delayed reflect DriveBC feed health, so they surface
            even when the thumbnail itself is unavailable. */}
        {isStale && (
...
export default function Cameras() {
  ...
  const cameraGroups = useMemo(() => {
    ...
    filteredCameras.forEach((camera) => {
      const roadName = camera.road?.name || 'Other';

      if (!groups.has(roadName)) {
        groups.set(roadName, []);
      }

      groups.get(roadName).push(camera);
    });

    return Array.from(groups.entries())
      .map(([roadName, cameras]) => ({
        roadName,
        ...
        <div className="camera-content-wrapper">
          ...
          <main className="camera-content">
            ...
            <div className="camera-groups">
              {cameraGroups.map(({ roadName, cameras }) => (
                <CameraHighwayGroup
                  key={roadName}
                  ...
                  onViewOnDriveBC={(camera) => {
                    const cameraViews =
                      camera.views || camera.cameraview_set || [];

                    const viewWithWebcam = cameraViews.find(
                      (view) => view.drivebc_webcam_id
                    );

                    if (!viewWithWebcam) {
                      console.warn(
                        'No DriveBC webcam id available for camera',
                        camera.id
                      );
                      return;
                    }
                    ...
                    window.open(
                      `https://drivebc.ca/cameras/${viewWithWebcam.drivebc_webcam_id}`,
                      '_blank',
                      'noopener,noreferrer'
                    );
                  }}
                  ...
                  onClone={async (camera) => {
                      try {
                        ...
                        navigate(`/cameras/${clonedCamera.id}/`);
                      } catch (error) {
